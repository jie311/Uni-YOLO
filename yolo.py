...
class YOLO(object):
    _defaults = {
        "model_path": 'model_data/COCO.pth',
    ...
    def uni_detect_image(self, image, crop=False, count=False):
        import os
        import clip
        import torch
        ...
        Cresult = []
        with open('./model_data/testclass.txt', 'r') as f:
            for line in f:
                Cresult.extend(list(line.strip('\n').split(',')))
        text_inputs = torch.cat([clip.tokenize(f"a photo of a {c}") for c in Cresult]).to(device)
        ...
        if crop:
            ...
            for i, c in list(enumerate(top_boxes)):
                ...
                crop_image = image.crop([left, top, right, bottom])
                crop_image.save(os.path.join(dir_save_path, "crop_" + str(i) + ".png"), quality=95, subsampling=0)
                image_CLIP = crop_image
                image_input = preprocess(image_CLIP).unsqueeze(0).to(device)
                with torch.no_grad():
                ...
                for value, index in zip(values, indices):
                    a = f"{Cresult[index]:>16s}: {100 * value.item():.2f}%"
                    Lab[i] = a
        for i, c in list(enumerate(top_label)):
            box = top_boxes[i]
            ...
            aaa = Lab[i].split(":", 1)[0].strip()

            if aaa in ["background", "object"]:
                print(aaa)
                continue
    ...
    def get_map_txt(self, image_id, image, class_names, map_out_path):
        ...
        f.close()
        return
    def uni_get_map_txt(self, image_id, image, class_names, map_out_path, text_features, Cresult, model, preprocess,
                        device):
        ...
        for i, c in list(enumerate(top_boxes)):
            ...
            if left >= right:
                left = right - 1
            image_CLIP = image.crop([left, top, right, bottom])
            image_input = preprocess(image_CLIP).unsqueeze(0).to(device)
            with torch.no_grad():
                image_features = model.encode_image(image_input)
            image_features /= image_features.norm(dim=-1, keepdim=True)
            similarity = (100.0 * image_features @ text_features.T).softmax(dim=-1)
            value, indices = similarity[0].topk(1)
            Lab[i] = f"{Cresult[indices]}"
            Labvalue[i] = value

        for i, c in list(enumerate(top_label)):
            predicted_class = Lab[i]
            box = top_boxes[i]
            score = str(top_conf[i])

            top, left, bottom, right = box

            if predicted_class in ["background", "object"]:
                continue
        ...
        f.close()
        return
    def add_label_txt(self, image_id, image, class_names, map_out_path, text_features, Cresult, model, preprocess,
                        device):
        ...
        for i, c in list(enumerate(top_label)):
            ...
            bottom = min(image.size[1], np.floor(bottom).astype('int32'))
            right = min(image.size[0], np.floor(right).astype('int32'))
            if predicted_class in ["airplane", "bus", "cat", "dog", "cow", "elephant", "umbrella", "tie", "snowboard", "skateboard", "cup", "knife", "cake", "couch", "keyboard", "sink", "scissors"]:
               predicted_class = "0"
               a = (left + right) / (2 * w)
               b = (top + bottom) / (2 * h)
               c = (right - left) / w
               d = (bottom - top) / h
               f.write("%s %s %s %s %s\n" % (
                   predicted_class, str(a), str(b), str(c), str(d)))
               image.save(os.path.join(map_out_path, "images/" + image_id + ".jpg"))
